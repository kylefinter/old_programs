...
int main(int argc, char **argv) {
  ...
  int comm_sz = argv[1]; //Number of processes
  ...
  if (my_rank != 0) {
    ...
    */
    ...
    //printf("process %d: %d\n", my_rank, local_n);
    //Allocate memory for local_nums

    MPI_Scatter(global_nums, local_n, MPI_INT, local_nums, local_n, MPI_INT, 0, MPI_COMM_WORLD);
    for (i = 0; i < local_n; i++) {
      local_sum += local_nums[i];
    }
    MPI_Reduce(&local_sum, &i, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    */
    ...
    //Get total number of integers in the file (use findlen)
    //Calculate number of ints handled by each process
    //Now broadcast local_n to all processes
    global_n = findlen(fname);
    local_n = global_n/comm_sz;
    MPI_Bcast(&local_n, 1, MPI_INT, 0, MPI_COMM_WORLD);
    //Allocate memory for all nums
    //Allocate memory for local nums
    Read_vector(local_nums, local_n, comm_sz, global_nums, 0, MPI_COMM_WORLD);
