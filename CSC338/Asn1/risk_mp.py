...
def simProcess(the_que, result):
    while not the_que.empty():
        cur = the_que.get()
        result.put(run_simulations(cur[0], cur[1], cur[2], cur[3]))
    return
...
def main(num_simulations):
    ...
    for sim in range(num_simulations):
        que.put((1, num_armies, num_attack_dice, num_defend_dice))
    for i in range(num_processors):
        p = Process(target = simProcess, name = "process "+str(i), args = [que, res])
        processList.append(p)
        p.start()
    print(len(processList), "procs")
    for p in processList:
        p.join()
    for i in range(num_simulations):
        results.extend(res.get())
    print(len(results), "sims")
